...
const createSchema = z.object({
  ...
  budget: z.coerce.number().positive().finite().max(1_000_000_000),
  currency: z.string().trim().toUpperCase().length(3).regex(/^[A-Z]{3}$/).default("SAR"),
});

const responseSchema = z.object({
  message: z.string().trim().min(10).max(2000),
  offerPrice: z.coerce.number().positive().finite().max(1_000_000_000),
});

rfqRouter.get("/", async (_req, res, next) => {
  try {
...
rfqRouter.post("/", requireAuth, async (req, res, next) => {
  try {
...
rfqRouter.post("/:id/respond", requireAuth, async (req, res, next) => {
  try {
